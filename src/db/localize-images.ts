...
export async function localizeRemoteImages(db: DB) {
  ...
  for (const item of items) {
    ...
    if (local) await db.update(menuItems).set({ imageUrl: local }).where(eq(menuItems.id, item.id));
  }
  ...
  for (const key of ["heroImageUrl", "aboutImageUrl"] as const) {
    ...
    if (local) await db.update(homepageContent).set({ [key]: local }).where(eq(homepageContent.id, 1));
  }
}
